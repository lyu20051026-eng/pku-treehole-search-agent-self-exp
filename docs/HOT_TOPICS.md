...
选择 `4`，输入 `4`、`8`、`12` 或 `24`（回车默认 24）。进入模式 4 时立即显示并固定截止时间，所选小时数从该时刻向前计算；输入选项和抓取期间截止时间不移动。采集开始前再次显示完整起止时间。程序扫描近期候选，按热词与互动筛选最多 30 帖，抓取评论，先导出素材，再输出话题概述、主要讨论和出处。材料足够时汇总 5～10 个话题，不强凑数量。完成后返回菜单。仅终端增加此模式，Web 和邮件入口仍为原有模式 1～3。

也可直接运行：

```bash
python agent.py --hot --hot-hours 24
python agent.py --hot-only --hot-hours 8
...
模型先做有出处的分块笔记，再汇总为目标 1500 字以内的报告；不足一个块时直接总结。先将明确的圆括号/方括号引用统一为 `[#ID]`；笔记引用某条评论时，后续汇总可以引用该评论所属主帖，但不会自动允许同帖其他评论或未读取帖子。笔记和最终报告都检查出处 ID 是否存在于结构化来源，并要求至少一个有效出处。格式、ID 校验失败或输出长度截断时最多纠正/压缩一次，仍不通过则报告失败；被拒草稿仅保存在本地 JSON 的 rejected_drafts，不能当作正式总结。纠正请求也计入最多 HOT_MAX_SUMMARY_CHUNKS+1 次总调用预算。该检查不能证明每一句话的语义得到原文支持，仍须人工抽查。超过素材块预算时标明模型覆盖比例；压缩仍超预算则明确失败，不能静默截断成完整报告。流式中断、空输出和长度截断不算成功。
...
1. 菜单中选择 `4`，默认 24 小时，确认导出最多 30 帖；选 `4`/`8`/`12` 时检查窗口起止时间。
...
`--hot-replay` 必须跟导出的 `posts.json` 路径（路径有空格时用引号包裹），例如 `python agent.py --hot-replay "data/hot_topics/某次运行/posts.json"`。重放保留原始统计窗口，不将历史素材标作当前时间的热点。

新采集不再支持 72/168 小时；已有的 72/168 小时 JSON 仍可重放，模型按原文件的窗口归纳。
